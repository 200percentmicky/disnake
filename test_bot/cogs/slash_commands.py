from typing import Optional

import discord
from discord.ext import commands


async def test_autocomp(inter, string):
    return ["XD", ":D", ":)", ":|", ":("]


class SlashCommands(commands.Cog):
    def __init__(self, bot):
        self.bot: commands.Bot = bot

    @commands.slash_command()
    async def hello(self, inter: discord.CommandInteraction):
        await inter.response.send_message("Hello world!")

    @commands.slash_command()
    async def auto(self, inter: discord.CommandInteraction, mood: str):
        """
        Has an autocomplete option.

        Parameters
        ----------
        mood: Dude
        """
        await inter.send(mood)

    @auto.autocomplete("mood")
    async def test_autocomp(self, inter: discord.CommandInteraction, string: str):
        return ["XD", ":D", ":)", ":|", ":("]

    @commands.slash_command()
    @commands.guild_permissions(768247229840359465, roles={815866581233041428: False})
    async def alt_auto(
        self,
        inter: discord.AppCmdInter,
        mood: str = commands.Param(autocomplete=test_autocomp),
    ):
        await inter.send(mood)

    @commands.slash_command()
    async def guild_only(self, inter: discord.GuildCommandInteraction, option: str = None):
        await inter.send(f"guild: {inter.guild} | option: {option!r}")

    @commands.slash_command()
    async def ranges(
        self,
        inter: discord.CommandInteraction,
        a: int = commands.Param(None, lt=0),
        b: commands.Range[1, ...] = None,
        c: commands.Range[0, 10] = None,
        d: commands.Range[0, 10.0] = None,
    ):
        """limit slash command options to a range of values

        Parameters
        ----------
        a: Negative
        b: Positive
        c: 0 - 10 integer
        d: 0 - 10 float
        """
        await inter.send(f"{inter.options}")


def setup(bot):
    bot.add_cog(SlashCommands(bot))
    print(f"> Extension {__name__} is ready\n")
